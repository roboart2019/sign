...
class SignOcaField(models.Model):
    ...
    def _get_model_field_choices(self, model_name):
        ...
        field" to auto-fill an item, for the given model.
        ...
        for fname, field in sorted(model._fields.items()):
            ...
            if field.type == "many2one" and field.comodel_name in self.env:
                ...
                for sub_fname, sub_field in sorted(related_model._fields.items()):
        ...
        return sorted(choices, key=lambda item: item["string"])
